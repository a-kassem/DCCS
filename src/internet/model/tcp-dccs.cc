...
namespace ns3
{
    ...
    NS_LOG_COMPONENT_DEFINE("TcpDccs");

    NS_OBJECT_ENSURE_REGISTERED(TcpDccs);
    ...
    TcpDccs::TcpDccs(const TcpDccs& sock)
            : TcpNewReno(sock),
    ...
    {
        NS_LOG_FUNCTION(this);
        return CopyObject<TcpDccs>(this);
    }

    void
    TcpDccs::Init(Ptr<TcpSocketState> tcb)
    {
...
// cwnd is set to ssthresh (this value).  bytesInFlight is ignored.
    uint32_t
    TcpDccs::GetSsThresh(Ptr<const TcpSocketState> tcb, uint32_t bytesInFlight) //TODO
    {
    ...
    void
    TcpDccs::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked, const Time& rtt) //TODO
    {
        ...
        if (tcb->m_lastAckedSeq >= m_nextSeq)
        {
            ...
            if(!m_drain_updated && m_count == 0)
            {
                ...
                m_drain_cycle = std::floor(0.6 * m_baseRtt.GetSeconds() * m_capacity / m_flow_num / tcb->m_segmentSize + (m_k - 10));
            }
    ...
    void
    TcpDccs::IncreaseWindow(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked) //TODO
    {
    ...
    void
    TcpDccs::InitializeEcnAlpha(double alpha)
    {
    ...
    void
    TcpDccs::InitializeDelayAlpha(double alpha)
    {
        NS_LOG_FUNCTION(this << alpha);
        NS_ABORT_MSG_IF(m_initialized, "DCCS has already been initialized");
        m_alpha_d = alpha;
    ...
    void
    TcpDccs::InitializeNQAvg(double nq)
    {
    ...
    void
    TcpDccs::Reset(Ptr<TcpSocketState> tcb)
    {
        NS_LOG_FUNCTION(this << tcb);
        m_nextSeq = tcb->m_nextTxSequence;
        m_ackedBytesEcn = 0;
    ...
    void
    TcpDccs::CeState0to1(Ptr<TcpSocketState> tcb)
    {
        NS_LOG_FUNCTION(this << tcb);
    ...
    void
    TcpDccs::CeState1to0(Ptr<TcpSocketState> tcb)
    {
        NS_LOG_FUNCTION(this << tcb);
        if (m_ceState && m_delayedAckReserved && m_priorRcvNxtFlag)
        {
    ...
    void
    TcpDccs::UpdateAckReserved(Ptr<TcpSocketState> tcb, const TcpSocketState::TcpCAEvent_t event)
    {
    ...
    void
    TcpDccs::CwndEvent(Ptr<TcpSocketState> tcb, const TcpSocketState::TcpCAEvent_t event)
    {
        NS_LOG_FUNCTION(this << tcb << event);
        switch (event)
        {
