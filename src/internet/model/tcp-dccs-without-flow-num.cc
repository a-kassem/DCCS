...
#include "tcp-dccs-without-flow-num.h"
...
namespace ns3
{
    ...
    NS_LOG_COMPONENT_DEFINE("TcpDccsNoFlow");

    NS_OBJECT_ENSURE_REGISTERED(TcpDccsNoFlow);

    TypeId
    TcpDccsNoFlow::GetTypeId()
    {
        static TypeId tid =
                TypeId("ns3::TcpDccsNoFlow")
                        .SetParent<TcpNewReno>()
                        .AddConstructor<TcpDccsNoFlow>()
                        .SetGroupName("Internet")
                        .AddAttribute("Capacity",
                                      "capacity of bottleneck",
                                      UintegerValue(1250000000),
                                      MakeUintegerAccessor(&TcpDccsNoFlow::m_capacity),
                                      MakeUintegerChecker<uint64_t>())
                        .AddAttribute("DccsShiftG",
                                      "Parameter G for updating alpha",
                                      DoubleValue(0.0625),
                                      MakeDoubleAccessor(&TcpDccsNoFlow::m_g),
                                      MakeDoubleChecker<double>(0, 1))
                        .AddAttribute("EcnAlphaOnInit",
                                      "Initial alpha_ecn",
                                      DoubleValue(0),
                                      MakeDoubleAccessor(&TcpDccsNoFlow::InitializeEcnAlpha),
                                      MakeDoubleChecker<double>(0, 1))
                        .AddAttribute("DelayAlphaOnInit",
                                      "Initial alpha_d",
                                      DoubleValue(0),
                                      MakeDoubleAccessor(&TcpDccsNoFlow::InitializeDelayAlpha),
                                      MakeDoubleChecker<double>(0, 1))
                        .AddAttribute("NQAvgOnInit",
                                      "Initial m_nq_avg",
                                      DoubleValue(0),
                                      MakeUintegerAccessor(&TcpDccsNoFlow::InitializeNQAvg),
                                      MakeDoubleChecker<double>(0, 100000))
                        .AddAttribute("Kdcv",
                                      "Upper bound of packets in network",
                                      UintegerValue(16),
                                      MakeUintegerAccessor(&TcpDccsNoFlow::m_k),
                                      MakeUintegerChecker<uint32_t>())
                        .AddAttribute("Gamma",
                                      "Limit on increase",
                                      UintegerValue(1),
                                      MakeUintegerAccessor(&TcpDccsNoFlow::m_gamma),
                                      MakeUintegerChecker<uint32_t>())
                        ...
                        .AddAttribute("NQW",
                                      ...
                                      DoubleValue(0.0625),
                                      MakeDoubleAccessor(&TcpDccsNoFlow::m_nq_w),
                                      MakeDoubleChecker<double>(0, 1))
                        .AddAttribute("UseEct0",
                                      "Use ECT(0) for ECN codepoint, if false use ECT(1)",
                                      BooleanValue(true),
                                      MakeBooleanAccessor(&TcpDccsNoFlow::m_useEct0),
                                      MakeBooleanChecker())
                        .AddTraceSource("CongestionEstimate",
                                        "Update sender-side congestion estimate state",
                                        MakeTraceSourceAccessor(&TcpDccsNoFlow::m_traceCongestionEstimate),
                                        "ns3::TcpDccs::CongestionEstimateTracedCallback");
        return tid;
    }

    std::string
    TcpDccsNoFlow::GetName() const
    {
        return "TcpDccsNoFlow";
    }

    TcpDccsNoFlow::TcpDccsNoFlow()
            : TcpNewReno(),
              m_ackedBytesEcn(0),
    ...
    TcpDccsNoFlow::TcpDccsNoFlow(const TcpDccsNoFlow& sock)
            : TcpNewReno(sock),
    ...
    {
        NS_LOG_FUNCTION(this);
        return CopyObject<TcpDccsNoFlow>(this);
    }

    void
    TcpDccsNoFlow::Init(Ptr<TcpSocketState> tcb)
    {
...
// cwnd is set to ssthresh (this value).  bytesInFlight is ignored.
    uint32_t
    TcpDccsNoFlow::GetSsThresh(Ptr<const TcpSocketState> tcb, uint32_t bytesInFlight) //TODO
    {
    ...
    void
    TcpDccsNoFlow::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked, const Time& rtt) //TODO
    {
        ...
        if (tcb->m_lastAckedSeq >= m_nextSeq)
        {
            ...
            {
                m_drain_cwnd = std::floor(0.6 * m_baseRtt.GetSeconds() * m_capacity);
                m_drain_cycle = std::floor(0.6 * m_baseRtt.GetSeconds() * m_capacity / tcb->m_segmentSize + (m_k - 10));
            }
    ...
    void
    TcpDccsNoFlow::IncreaseWindow(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked) //TODO
    {
    ...
    void
    TcpDccsNoFlow::InitializeEcnAlpha(double alpha)
    {
    ...
    void
    TcpDccsNoFlow::InitializeDelayAlpha(double alpha)
    {
        NS_LOG_FUNCTION(this << alpha);
        NS_ABORT_MSG_IF(m_initialized, "DCCS has already been initialized");
        m_alpha_d = alpha;
    ...
    void
    TcpDccsNoFlow::InitializeNQAvg(double nq)
    {
    ...
    void
    TcpDccsNoFlow::Reset(Ptr<TcpSocketState> tcb)
    {
        NS_LOG_FUNCTION(this << tcb);
        m_nextSeq = tcb->m_nextTxSequence;
        m_ackedBytesEcn = 0;
    ...
    void
    TcpDccsNoFlow::CeState0to1(Ptr<TcpSocketState> tcb)
    {
        NS_LOG_FUNCTION(this << tcb);
    ...
    void
    TcpDccsNoFlow::CeState1to0(Ptr<TcpSocketState> tcb)
    {
        NS_LOG_FUNCTION(this << tcb);
        if (m_ceState && m_delayedAckReserved && m_priorRcvNxtFlag)
        {
    ...
    void
    TcpDccsNoFlow::UpdateAckReserved(Ptr<TcpSocketState> tcb, const TcpSocketState::TcpCAEvent_t event)
    {
    ...
    void
    TcpDccsNoFlow::CwndEvent(Ptr<TcpSocketState> tcb, const TcpSocketState::TcpCAEvent_t event)
    {
        NS_LOG_FUNCTION(this << tcb << event);
        switch (event)
        {
